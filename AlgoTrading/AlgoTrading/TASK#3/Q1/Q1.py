...
def w_closed_form(X, Y):
    ...
    XtX_inv = torch.linalg.inv(XtX)  # (X^T X)^-1
    XtY = Xt.mm(Y)  # X^T Y
    ...
    w_closed = w_closed.detach().numpy().squeeze(axis=1)#w_closed to a numpy array(1 dimensional)
...
def l2_loss(X, Y, w):
    ...
    if X.shape[1] is not w.shape[0]:
        X = torch.cat([torch.ones((X.shape[0], 1)),X],dim = 1)#SHAPE BECOMES (n.d+1)
    w = w.double()#converts it into double precision float
