...
def w_closed_form(X, Y):
    ...
    # Student code end
    X = torch.cat([torch.ones((X.shape[0], 1)),X],dim = 1)#SHAPE BECOMES (n.d+1)
    X = X.to(torch.float32)
    Y = Y.to(torch.float32)
    # Round w_closed upto 4 decimal places
    Xt = X.t()  # Transpose of X
    XtX = Xt.mm(X)  # X^T X: mm is used for representing matrix multitplication /dot product
    lambda_value = 0.01#to reduce the odds of singular matrices
    I = torch.eye(X.shape[1])
    XtX_reg = XtX + lambda_value * I
    XtX_inv = torch.linalg.inv(XtX_reg)  # (X^T X)^-1
    
    XtY = Xt.mm(Y)  # X^T Y
    w_closed = XtX_inv.mm(XtY)
...
def l2_loss(X, Y, w):
    ...
    '''
    if X.shape[1] is not w.shape[0]:
        X = torch.cat([torch.ones((X.shape[0], 1)),X],dim = 1)#SHAPE BECOMES (n.d+1)
    #converts it into double precision float
    loss = torch.mean((Y - X.mm(w))**2)
...
def train_model(X_train, Y_train, X_test, Y_test, w, eta):
    ...
    while abs(l2_loss(X_train , Y_train, w)-old_loss)>epsilon:
        ...
        gradient = l2_loss_derivative(X_train, Y_train, w)
        w = w - eta*gradient
...
if __name__ == '__main__':
    ...
    possible_degrees = range(1, 10)
#WE HAVE TO ASSUME THAT X VECTOR IS 1 DIMENSIONAL ,RIGHT?
